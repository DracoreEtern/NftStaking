...
import "https://github.com/DracoreEtern/NftStaking/Etern.sol";
import "https://github.com/DracoreEtern/NftStaking/EterNft.sol";
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(EterNft _nft, Etern _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      earned += 10 ether * (block.timestamp - stakedAt) / 86400;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
  ...
  function earningInfo(uint256[] calldata tokenIds) external view returns (uint256[2] memory info) {
     ...
     uint256 earned = 0;
      ...
      uint256 stakedAt = staked.timestamp;
      earned += 10 ether * (block.timestamp - stakedAt) / 86400;
    uint256 earnRatePerSecond = totalScore * 10 ether / 86400;
